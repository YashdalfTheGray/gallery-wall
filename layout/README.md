...
Place picture frames of mixed sizes and shapes around a centerpiece. The centerpiece center is at `(0, 0)`. Every other frame attaches edge-to-edge with a configurable gap to form one connected cluster.

This directory is a standalone Go module. You can depend on it without the web app, WASM build, or CLI tools in the parent repository.
...
Tag releases on the parent repository as `layout/vX.Y.Z` (for example `layout/v1.0.0`).
...
`Params`, `Result`, and error types marshal and unmarshal JSON for HTTP APIs and config files. See `example_test.go` for round-trip examples.
...
Geometry, candidate generation, and scoring helpers are exported for tests and tuning. Use `Layout` in application code. Full godoc is in `doc.go`.
...
From the repository root (requires `go.work` in a full clone):
